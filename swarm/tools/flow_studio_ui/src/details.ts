...
import type {
  ...
  AgentUsage,
  StepTiming,
  StepDetailsCallbacks,
  AgentDetailsCallbacks,
  AgentUsageCallbacks,
  FlowStep,
...
import {
  renderSelectNodeHint,
  renderAgentUsageItem,
  renderAgentLocationInfo,
 ...
 */
export async function renderRunTimeline(container: HTMLElement): Promise<void> {
  ...
  try {
    ...
    try {
      const timingData = await Api.getRunTiming(state.currentRunId) as TimingData;
 ...
 */
export async function renderFlowTiming(container: HTMLElement, flowKey: FlowKey): Promise<void> {
  if (!state.currentRunId || !flowKey) {
    return;
  }

  try {
    const data = await Api.getFlowTiming(state.currentRunId, flowKey) as TimingData;
 ...
 */
export function renderStepTiming(timing: StepTiming): string {
        ...
        setTimeout(() => {
          if (state.cy) {
            ...
            if (node) {
              ...
              node.select();
              if (showStepDetails) {
                showStepDetails(node.data() as NodeData);
              }
            }
          }
        }, 300);
 ...
 */
function renderTeachingCallout(teachingNote: string): string {
  if (!teachingNote) return "";
 ...
 * Only visible when Teaching Mode is enabled.
 */
function renderStructuredTeachingNotes(notes: { inputs?: string[]; outputs?: string[]; emphasizes?: string[]; constraints?: string[] } | undefined): string {
  if (!notes) return "";

  const hasContent =
    (notes.inputs?.length ?? 0) > 0 ||
 ...
 */
function renderTranscript(resp: StepTranscriptResponse): string {
 ...
 */
async function loadReceiptBadges(
  container: HTMLElement,
  runId: string,
  flowKey: FlowKey,
  stepId: string
 ...
 */
function renderContextTruncation(truncation: StepReceipt['context_truncation'] | undefined): string {
 ...
 */
function renderRoutingDecision(routing: { loop_iteration: number; max_iterations: number | null; decision: string; reason: string } | undefined): string {
  if (!routing) return "";
 ...
 */
export async function showStepDetails(
  data: StepNodeData,
  callbacks: StepDetailsCallbacks = {}
): Promise<void> {
  const {
    renderSelftestTab,
  ...
  let stepAgents: string[] = [];
  let stepDescription = "";
  let stepTeachingNote = "";
  if (data.flow) {
    try {
      const flowDetail = await Api.getFlowDetail(data.flow as FlowKey);
      const stepInfo = flowDetail.steps?.find((s: FlowStep) => s.id === data.step_id);
      if (stepInfo) {
    ...
    if (stepAgents.length === 0) {
      ...
      stepAgents.forEach(agentKey => {
        ...
        agentLink.addEventListener("click", async () => {
          if (selectAgent) {
            await selectAgent(agentKey, data.flow as FlowKey);
          }
        });
  ...
  const flowData = state.runStatus.flows?.[data.flow as FlowKey];
  ...
  if (state.currentRunId && data.flow && data.step_id) {
    ...
    if (badgesContainer) {
      loadReceiptBadges(
        badgesContainer as HTMLElement,
        state.currentRunId,
        data.flow as FlowKey,
        data.step_id
      );
    }
  }
  ...
  if (getNodeGovernanceInfo && renderGovernanceSection) {
    const govInfo = getNodeGovernanceInfo(data as NodeData);
    if (govInfo) {
 ...
 */
export async function showAgentDetails(
  data: AgentNodeData,
  callbacks: AgentDetailsCallbacks = {}
): Promise<void> {
  const {
    setActiveFlow,
  ...
  try {
    const usageData = await Api.getAgentUsage(agentKey) as { usage?: AgentUsage[] };
 ...
 */
export function showArtifactDetails(data: ArtifactNodeData): void {
  const detailsEl = document.getElementById("details");
  if (!detailsEl) return;
  detailsEl.innerHTML = "";
