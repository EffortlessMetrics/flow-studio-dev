...
export const screens: ScreenSpec[] = [
  {
    ...
    title: "Flows - Default",
    description:
      "Main Flow Studio screen with run selector, flow list, graph canvas, and inspector.",
    regions: [
      {
        id: "header",
      ...
      },
      {
        id: "sidebar",
        purpose:
          "Run selector, flow list, and view toggles between agents and artifacts.",
        uiids: [
          "flow_studio.sidebar",
      ...
      },
      {
        id: "inspector",
        purpose:
          "Details panel for selected step/agent/artifact, timing, and timeline.",
        uiids: [
 ...
 */
export const screenById: Record<ScreenId, ScreenSpec> = Object.fromEntries(
  screens.map((s) => [s.id, s])
) as Record<ScreenId, ScreenSpec>;

/**
 * Get all known screen IDs.
 */
export function getScreenIds(): ScreenId[] {
 ...
 */
export function getScreenById(id: ScreenId): ScreenSpec | null {
 ...
 */
export function getAllKnownUIIDs(): FlowStudioUIID[] {
  const uiids = new Set<FlowStudioUIID>();
 ...
 */
export function validateLayoutUIIDs(): { valid: boolean; issues: string[] } {
