...
import type { GraphState, SerializedNode, SerializedEdge, ArtifactStatus } from "./domain.js";
 ...
 */
export function getCurrentGraphState(): GraphState | null {
  if (!state.currentFlowKey || !state.cy) return null;

  // Sort nodes by ID for deterministic output
  const nodes: SerializedNode[] = state.cy.nodes()
    .map((node) => ({
      id: node.id(),
      type: node.data("type") as SerializedNode["type"],
      label: node.data("label") as string,
      flow: node.data("flow") as SerializedNode["flow"],
      step_id: (node.data("step_id") as string | null) || null,
      order: (node.data("order") as number | null) || null,
      color: (node.data("color") as string | null) || null,
      status: (node.data("status") as ArtifactStatus | null) || null
    }))
    .sort((a, b) => a.id.localeCompare(b.id));

  // Sort edges by ID for deterministic output
  const edges: SerializedEdge[] = state.cy.edges()
    .map((edge) => ({
      id: edge.id(),
      type: edge.data("type") as SerializedEdge["type"],
 ...
 */
export function renderFlowOutline(): void {
